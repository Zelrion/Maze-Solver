...
public class Maze:MonoBehaviour {
	...
	public bool perfectMaze = true; //Require the result be Perfect; One Solution
	...
	public void MergeMeshes() {
		if(meshMaster == null) { //If a Mesh Container GameObject hasn't been made, make one
			meshMaster = new GameObject("Meshes");
			meshMaster.transform.parent = transform;
		}
		...
		int trisCount = 0, filterIndex = 0; //Triangle Count of the Mesh, Current Index in MeshFilter Array
		for(int mesh = 0;mesh < meshes.Count;mesh++) { //For every Mesh (Meshes can be added if current exceeds 65k Tris, causing an extra iteration)
			...
			GameObject obj = new GameObject(); //Create the Object to be Made
			obj.AddComponent<MeshFilter>().mesh = meshes[mesh]; //Add the MeshFilter and Set Mesh (Shape)
			obj.AddComponent<MeshRenderer>().material = wallMaterial; //Add the MeshRenderer and Set Material (Color/Texture/Shader/Etc)
			obj.transform.parent = meshMaster.transform; //Make the Object a child of the Mesh Master
			for(int i = 0;filterIndex < meshFilters.Length;i++) { //Iterate the MeshFilters from Cells
				MeshFilter mf = meshFilters[filterIndex++]; //Set Current MeshFilter
				if(trisCount + mf.mesh.triangles.Length < 65000) { //Check if the Sum of the current count of Triangles and the Next Mesh is less than the Max Number of Triangles
					mf.gameObject.SetActive(false); //Set the Cell to InActive so it won't be found in the next call for children (Cell Master Children)
					combine[i].mesh = mf.sharedMesh; //Set the Combine Instance Mesh to the Current MeshFilter's (MF)
					combine[i].transform = mf.transform.localToWorldMatrix; //Transform from local to world space
					trisCount += mf.mesh.triangles.Length; //Add the Triangle count
				} else { //If sum exceeded 65k Triangles
					meshes.Add(new Mesh()); //Add another Mesh to force another Iteration
					break; //Break current loop
				}
			}
			obj.name = "Tris:" + trisCount; //Name the Mesh based on it's Triangle Count
			...
			trisCount = 0; //Reset Triangle Count to 0 for Next Mesh (Unessesary if Triangle Count never exceeded 65k)
		}
	}
	...
	private Cell CreateCell(IntVector2 coordinates) { //Instantiate a new Cell at Coordinates
		if(cellMaster == null) { //If a Cell Container GameObject hasn't been made, make one
			cellMaster = new GameObject("Cells");
			cellMaster.transform.parent = transform;
		}
